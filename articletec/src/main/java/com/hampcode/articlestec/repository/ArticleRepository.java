...
public interface ArticleRepository extends PagingAndSortingRepository<Article, Long> {
	...
	List<Article> findByAuthor(String author); //esto lo convierte automaticamente
	
	@Query("SELECT MAX(id) FROM Article")
	Long findTopByOrderByIdDesc();
	
	@Query("SELECT FROM Article a WHERE a.title=:title and a.author=:author")
	List<Article> findByTitleAndAuthor(@Param("title")String title,
			@Param("author")String author);
	
	Page<Article> findAll(Pageable pageable); //te devuelve una pagina relacionada con el pageable
